/**
 * JSONEditor でのデフォルトテンプレートが go template の {{}} のリテラルとかにばっているため更新しています。
 */
JSONEditor.defaults.options.template = {
    compile: (template) =>
        vars => {
            return template.replace(/{self}/g, vars.self);
        }
};

JSONEditor.defaults.language = "jp";
JSONEditor.defaults.languages.jp =  {
    /**
     ...
     * @variables This key takes one variable: The maximum
     */
    error_maximum_incl: '{{0}}以下を設定してください',
    /**
     ...
     * @variables This key takes one variable: The minimum
     */
    error_minimum_incl: '{{0}}以上を設定してください',
    /**
     ...
     */
    error_uniqueItems: '重複した内容が登録されています',
    /**
     ...
     */
    button_move_down_title: '一つ下へ移動',
    /**
     * Title on Move Up buttons
     */
    button_move_up_title: '一つ上へ移動',
    /**
     ...
     */
    button_delete_node_warning: '削除します。よろしいですか？',
    /**
     * Warning when deleting a node
     */
...
JSONEditor.defaults.custom_validators.push((schema, value, path) => {
    const errors = [];
    if (schema.format === "date" && schema.options?.flatpickr?.mode === "range") {
        ...
        if (!startDate.getTime() || !endDate.getTime()) {
            errors.push({
                path: path,
                property: 'format',
                message: '`YYYY-MM-DD to YYYY-MM-DD` の形式で記載してください',
            });
        }
        if (startDate.getTime() > endDate.getTime()) {
            errors.push({
